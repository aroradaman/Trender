...
def mongo(body) :
	...
	db = client.Central
	if body['increment'] :
		print 'True' , body.keys(),'daman'
		track = db.Data.find_one({'id':body['id']})
		print 'keys',track.keys(),track['played'],'track'
		try :
			newPlayedArr = track['played']
			newPlayedArr.append(body['time'])
			db.Data.update({'_id':body['id']},{'$set':{'played':newPlayedArr}},upsert=True)
			print body['id']
		except Exception as error :
			pass
	else :
		print 'False' , body.keys()
		if db.Data.find({'_id':body['id']}).count() == 0 :
			db.Data.save({'_id':body['id'],'id':body['id'],'title':body['title'],'artist':body['artist'],'genre':body['genre'],'year':body['year'],'album':body['album'],'location':[body['ip']],'played':[time.time()]})
		else :
			try :
				data = db.Data.find_one({'_id':body['id']})
				newLocArr = data['location']
				newLocArr.append(body['ip'])
				newPlayedArr = data['played']
				newPlayedArr.append(time.time())
				db.Data.update({'_id':body['id']},{'$set':{'location':newLocArr,'played':newPlayedArr}},upsert=True)
			except Exception as error :
				pass
...
def idIndexer(dataSet) :
	...
	cur = con.cursor()
	sql = 'CREATE TABLE IF NOT EXISTS Data (id varchar(100), title varchar(100), artist varchar(100), album varchar(100), genre varchar(100), year varchar(100) , path varchar(200) , played varchar(10000) , ctime varchar(100))'
	cur.execute(sql)
...
def handleData(pathList) :
	...
	for path in pathList :
		...
		if len(ret) == 0 :
			sql = 'INSERT INTO Data VALUES (?,?,?,?,?,?,?,?,?)'
			cur.execute(sql,(Id,uni(tags[0]),uni(tags[1]),uni(tags[2]),uni(tags[3]),uni(tags[4]),path,json.dumps([]),getPathTime(path)))
			con.commit()
			mongo({'id':Id,'title':uni(tags[0]),'artist':uni(tags[1]),'album':uni(tags[2]),'genre':uni(tags[3]),'year':uni(tags[4]),'ip':SYS_IP,'increment':False})
			print tags[0]
			dataIndexer(Id,uni(tags[0]))
			idIndexer([Id,uni(tags[0]),uni(tags[1]),uni(tags[2]),uni(tags[3]),uni(tags[4]),path,json.dumps([]),getPathTime(path)],)
	con.close()
